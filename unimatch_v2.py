...
def main():
    ...
    # local_rank = int(os.environ["LOCAL_RANK"])  # ❌ 这会导致 KeyError
    local_rank = 0  # ✅ 直接使用 GPU 0
    model.cuda()
    ...
    trainloader_l = DataLoader(
        trainset_l, batch_size=cfg['batch_size'], pin_memory=False, num_workers=0, drop_last=True,
        # sampler=RandomSampler(trainset_l) pin_memory=True
        shuffle=True
    )

    trainloader_u = DataLoader(
        trainset_u, batch_size=cfg['batch_size'], pin_memory=False, num_workers=0, drop_last=True,
        # sampler=RandomSampler(trainset_u)
        shuffle=True
    )

    valloader = DataLoader(
        valset, batch_size=1, pin_memory=False, num_workers=0, drop_last=False,
        # shuffle=False
        shuffle=True
    )
